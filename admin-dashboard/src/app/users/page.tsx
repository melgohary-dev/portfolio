        ...
        <table className="w-full text-start text-sm">
          <thead className="border-b border-slate-100 bg-slate-50 text-xs uppercase tracking-wide text-slate-700 dark:border-slate-800 dark:bg-slate-800/50 dark:text-slate-400">
            <tr>
              ...
              <th className="px-5 py-3 font-medium">{t("users.status")}</th>
              <th className="px-5 py-3 font-medium">{t("users.created")}</th>
            </tr>
          </thead>
              ...
              >
                ...
                <td className="px-5 py-3">
                  <span className="rounded-full bg-blue-50 px-2.5 py-0.5 text-xs font-medium text-blue-700 ring-1 ring-blue-100 dark:bg-blue-950/40 dark:text-blue-300 dark:ring-blue-900">
                    {t(`role.${user.role.toLowerCase()}` as never)}
                  </span>
                </td>
                <td className="px-5 py-3">
                  <StatusBadge
                    status={user.status}
                    label={t(`status.${user.status}` as never)}
                  />
                </td>
                <td className="px-5 py-3 text-slate-600 dark:text-slate-400">
                  {user.createdAt}
